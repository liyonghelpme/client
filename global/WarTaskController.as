class WarTaskController extends ContextObject{
    ...
    function WarTaskController(){
        ...
        wartasktype =-1;
        wartasklib = dict();
        wartasklib.update(0,json_loads("{'id':0,'type':0,'req':'warinfo','pair':[],'des':['开启战争模式','升到第6级之后可以开启战争模式，你将能与其他玩家一起角逐帝王之路！还在等什么？赶快升到第6级吧！！','开启战争模式','提示：首先点击地图按钮，再点击战争图标'],'checknum':1,'reward':[5000,10,0]}"));
    ...
    function wartaskcomplete(n,e,p){
        ...
        if(p==1){
            ...
            ppy_postnewsfeed(ppy_username()+"完成了"+wartaskdes[0]+"任务，赶快加入与"+ppy_username()+"一起打造属于自己的奇迹帝国吧！", ShareURL, null);
        }
